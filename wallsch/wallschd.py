...
class WallpaperScheduler(object):
    ...
    def set_day(self, is_day):
        ...
        self.is_day = is_day
        if is_day:
            self.current_path_dir = self.wallpaper_dir/Path(self.day_subdir)
        else:
            self.current_path_dir = self.wallpaper_dir/Path(self.night_subdir)
    ...
    def update_wallpaper(self):
        ...
        '''
        if self.is_day:
            current_wallpaper = random.choice(self.filelist['day'])
        else:
            current_wallpaper = random.choice(self.filelist['night'])

        wallpaper_to_set = "{0}/{1}".format(
                self.current_path_dir.absolute().as_posix(),
                current_wallpaper)

        # log
        if config.VERBOSE:
            print(f'Path dir: {self.current_path_dir.absolute().as_posix()}')
